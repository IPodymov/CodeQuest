  ...
  <main class="flex-1 w-full px-4 sm:px-6 lg:px-8 py-8 flex justify-center">
    <div v-if="userStore.user" class="w-full max-w-[1100px] flex flex-col gap-8">
      <section class="rounded-xl bg-surface-dark border border-surface-highlight p-6 sm:p-8 shadow-sm">
        <div class="flex flex-col md:flex-row gap-6 md:items-center justify-between">
          <div class="flex items-center gap-6">
            <div class="bg-center bg-no-repeat bg-cover rounded-full size-24 sm:size-32 ring-4 ring-background-dark bg-slate-700"
              :style="{ backgroundImage: `url('${userStore.user.avatar || 'https://ui-avatars.com/api/?background=2b8cee&color=fff&size=128&name=' + userStore.user.username}')` }"></div>
          ...
          <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto mt-4 md:mt-0">
            <button @click="isEditOpen = true" class="flex items-center justify-center gap-2 rounded-lg h-10 px-6 bg-surface-highlight hover:bg-[#2e455c] text-white text-sm font-bold transition-colors w-full sm:w-auto">
      ...
      <section class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="bg-surface-dark border border-surface-highlight rounded-xl p-6 flex flex-col gap-2">
          <p class="text-sm font-medium uppercase tracking-wider text-text-secondary">{{ i18n.t('profile.participations') }}</p>
    ...
    <EditProfileModal :is-open="isEditOpen" @close="isEditOpen = false" />
  </main>
</template>
