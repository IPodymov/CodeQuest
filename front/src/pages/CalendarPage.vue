...
const registerForEvent = () => {
    if (!userStore.user) {
        if(confirm('Для регистрации необходимо войти. Перейти на страницу входа?')) {
            router.push('/auth');
        }
    } else {
  ...
  <div class="flex flex-1 w-full max-w-[1440px] mx-auto p-4 md:p-8 flex-col gap-8 relative">
     ...
     <div v-if="isAddModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="bg-[#111a22] border border-surface-border w-full max-w-md rounded-2xl shadow-2xl p-6 relative animate-fadeIn z-10">
           <h3 class="text-xl font-bold text-white mb-6">Добавить событие</h3>
           <div class="flex flex-col gap-4">
               <input placeholder="Название" class="w-full h-10 rounded-lg bg-surface-dark border-surface-border text-white px-3 text-sm" />
               <button @click="saveNewEvent" class="w-full h-10 bg-primary text-white font-bold rounded-lg mt-2">Сохранить</button>
           </div>
        </div>
     </div>
  </div>
