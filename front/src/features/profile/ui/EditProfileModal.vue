  ...
  <BaseModal :is-open="isOpen" @close="emit('close')" class="max-w-md p-6">
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="flex gap-3 mt-2">
        <button @click="save" class="flex-1 bg-primary hover:bg-primary-dark text-white font-bold py-2 rounded-lg">Сохранить</button>
        <button @click="emit('close')" class="flex-1 bg-surface-dark hover:bg-surface-highlight text-white font-bold py-2 rounded-lg border border-surface-highlight">Отмена</button>
      </div>
    </div>
  </BaseModal>
</template>
